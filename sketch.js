...
function constructVornoiDiagram() {
  // Calculate and store all info regarding the vornoi diagram
  ...
  // Gather data about the vornoi borders and neighbors. Basic principle is to have a border connecting circumcenters of triangles which share an edge with one another. Note this process gets tedious since we gotta consider intercepts to the screen. Also note that the neighbors "works" but may lead to bugs.
  ...
  for (let i = 0; i < length; i++) {
    ...
    for (let i = 0; i < triangle.vertices.length; i++) {
      ...
      if (adjM[nextV][curV]) {
        ...
        // An out case is bad bc the circumcenter may lie extremely outside of the screen which screws up processes such as lloyd relaxation. In this case, we would have a line that cn go from the center of the screen to smt like (-1000000000, 100000000). To fix this, we snip of the line at the point where is intersects with the box
        // in-in: both circumcenters lie in side screen. Simpist case so where we just connect the circumcenters directly

        let p1 = null;
        let p2 = null; // Points to draw the "contained" line
...
function keyPressed() {
  ...
  if (key == "r") {  // Adds 20 random seeds into the vornoi diagram
    for (let i = 0; i < 20; i++) {
